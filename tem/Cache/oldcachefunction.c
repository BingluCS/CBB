...
struct HPCsystem config = {
    ...
    .BB0 = "/root//CSD/BBcsd/",
    .BB1 = "/root/HPC/BB/cbb/",
    .json_file = "/root/HPC/Cache/file_cache_cbb.json",
...
char *demotion() {
    ...
    sprintf(command, "cp -r %s%s %s\nrm %sbk_%s -rf\nmv %s%s %sbk_%s\n",BurstBuffer, coldestfile, PFS, BurstBuffer, coldestfile, BurstBuffer, coldestfile, BurstBuffer,coldestfile);
...
int Write_file_cache(const char *filepath) {
    ...
    if(BB!=BBcsd && flag!=0) {
        char command[200];
        sprintf(command, "mv %sbk_%s %s%s",BB, filename, BB, filename);
        int status = system(command);
        if (status == 0) {
